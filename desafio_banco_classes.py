...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor):
        total_saques = len(
            [operacao for operacao in self.historico.transacoes if operacao["tipo"] == Saque.__name__]
        )

        limite_excedido = valor > self._limite
        saques_excedidos = total_saques >= self._limite_saques
...
class Saque(Transacao):
    ...
    def registrar(self, conta):
        operacao_bem_sucedida = conta.sacar(self.valor)
...
class Deposito(Transacao):
    ...
    def registrar(self, conta):
        operacao_bem_sucedida = conta.depositar(self.valor)
...
def buscar_cliente(cpf, clientes):
    lista_filtrada = [cliente for cliente in clientes if cliente.cpf == cpf]
    return lista_filtrada[0] if lista_filtrada else None


def conta_cliente_recuperar(cliente):
...
def depositar(clientes):
    ...
    conta = conta_cliente_recuperar(cliente)
    if not conta:
        return
